...
LAMBDAS = [round(v, 1) for v in np.arange(0.1, 1.0, 0.1)]   # 0.1 … 0.9
WINDOW_SIZE = 3
N_RUNS = 5
INIT_METHOD = 'kmeans_plusplus'
MAX_ITER = 100

SUMMARY_FILE = open('lambda_analysis_summary.txt', 'w', encoding='utf-8')
...
def run_dataset(dataset_name, n_clusters):
    log("\n" + "=" * 80)
    log(f"DATASET: {dataset_name.upper()}   k={n_clusters}   window_size={WINDOW_SIZE}")
    log("=" * 80)
    ...
    for lam in LAMBDAS:
        ...
        for run_idx in range(N_RUNS):
            ...
            log(f"      utility={u:.5f}  fairness={f:.5f}  ({elapsed:.1f}s)")
        ...
        log(f"    → avg utility={results[lam]['utility_mean']:.5f} ± {results[lam]['utility_std']:.5f}"
            f"   avg fairness={results[lam]['fairness_mean']:.5f} ± {results[lam]['fairness_std']:.5f}")
...
def plot_tradeoff(all_results):
    ...
    fig.suptitle("Fairness-Utility Trade-off in Stanley's Algorithm\n"
                 "(k and window_size fixed, λ varied 0.1→0.9)",
    ...
    for ax, dataset_name in zip(axes.flat, DATASETS):
        ...
        ax.set_title(f'{dataset_name.upper()}  (k={CLUSTERS_CONFIG[dataset_name]})',
                     fontsize=12)
        ax.set_xlabel('Lambda (λ) — Importance of Fairness', fontsize=11)
...
if __name__ == '__main__':
    start = time.time()
    log(f"\n{'='*80}")
    log("  STANLEY'S ALGORITHM — LAMBDA HYPERPARAMETER ANALYSIS")
    log(f"{'='*80}")
    ...
    for dataset_name in DATASETS:
        ...
        all_results[dataset_name] = run_dataset(dataset_name, n_clusters)
    ...
    for dataset_name in DATASETS:
        log(f"\n{dataset_name.upper()}:")
        log(f"  {'Lambda':>8}  {'Utility Mean':>14}  {'Utility Std':>12}  "
            f"{'Fairness Mean':>14}  {'Fairness Std':>12}")
        log(f"  {'-'*8}  {'-'*14}  {'-'*12}  {'-'*14}  {'-'*12}")
        for lam in sorted(all_results[dataset_name].keys()):
            r = all_results[dataset_name][lam]
            log(f"  {lam:>8.1f}  {r['utility_mean']:>14.6f}  {r['utility_std']:>12.6f}  "
                f"{r['fairness_mean']:>14.6f}  {r['fairness_std']:>12.6f}")
    df = pd.DataFrame.from_dict(all_results, orient="index")
    df.to_csv("Lambdaanalysis.csv")
